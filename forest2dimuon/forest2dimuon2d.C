...
void forest2dimuon2d(
	...
	TCanvas *c1 = new TCanvas("c1", "DiMuon 2D", 1200, 800);
	TH2F *dimu_h = new TH2F("", "", 20, 3, 3.2, 20, 0, 20);
	dimu_h->SetStats(kFALSE);
        dimu_h->GetXaxis()->SetTitle("m_{#mu#mu}[GeV/c^{2}]"); 
        dimu_h->GetYaxis()->SetTitle("Pt [GeV]");
	...
	for (int i=1;i<nMu;i++){
		...
		dimu=v1+v2;
		dimu_h->Fill(dimu.M(), dimu.Pt());
		}
	}
	} //end of event loop
	
	dimu_h->Draw("colz");
	c1->Print("diMuon_2d.png");
} 
