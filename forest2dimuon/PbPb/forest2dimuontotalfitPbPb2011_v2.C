...
void forest2dimuontotalfitPbPb2011_v2(
	...
	RooRealVar sigma_psi2s("sigma_psi2s","sigma_psi2s", 0.07, 0, 0.1);
	...
	RooRealVar NJpsi("NJpsi", "Numero de eventos de J/Psi",0, 860); 
	RooRealVar NPsi2S("NPsi2S", "Numero de eventos de Psi(2S)", 0, 60);
	RooRealVar Nbkg("Nbkg", "Numero de eventos de background", 0, 500);
 	  
 	RooRealVar fsig("fsig", "signal", 0.03, 0., 1.); 
    ...
    //sum.fitTo(dh,Range(2.6, 4));
         
    RooPlot* mframe = x.frame(Title("Fit of invariant mass in 2011 PbPb collision"));
 	dh.plotOn(mframe);
    ...
    latex->DrawLatex(0.15, 0.78, "PbPb 2011 #sqrt{s_{NN}} = 2.76 TeV");
    latex->DrawLatex(0.15, 0.74, "P^{#mu}_{T} > 0.0 GeV #lbar#eta^{#mu}#lbar < 2.4");
    //latex->DrawLatex(0.15, 0.70, "#lbar#eta#lbar < 2.4");
    //latex->DrawLatex(0.15, 0.66, "Trigger: HLT_HIL1DoubleMuOpen");
	
	 ca->Print("diMuon_mass_2011_PbPb_1.png");
