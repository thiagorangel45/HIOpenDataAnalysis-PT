...
void forest2dimuonCB_v6(
	...
	for (int i=1;i<nMu;i++){
		...
		dimu=v1+v2;
		if (dimu.M()<2.7) continue;
        	if (dimu.M()>3.4) continue;
		dimu_h->Fill(dimu.M());
		}
	...
	RooCBShape *jpsi = new RooCBShape("jpsi", "crystal ball PDF", x, mean_jpsi, sigma_jpsi, alpha, n);
 	 ...
 	 RooRealVar a("a", "a", -1, -5, 5); // -0.05, -0.03, -001, -0.1
 	 RooExponential bkg("exp", "exp", x, a);
 	 
 	 RooRealVar fsig("fsig", "signal", 0.9, 0., 1.); // aumentar o pico da CB
	 ...
	 sum.getParameters(dh)->setAttribAll("Constant",kTRUE) ;
 	 ...
 	 dh.plotOn(mframe); 
        ...
        latex->DrawLatex(0.15, 0.72, "pp 2011 #sqrt{s_{NN}} = 2.76 TeV");
        latex->DrawLatex(0.15, 0.66, "P^{#mu}_{T} > 2.0 GeV #lbar#eta#lbar < 2.4");
        ...
        label.SetTextSize(0.04);  // Define o tamanho do texto
	label.SetTextAngle(90);  // Define o ângulo do texto como 90 graus (vertical)
	// Adiciona um rótulo ao título do eixo y
	label.DrawLatex(0.045, 0.55, "Events/(0.02 GeV/c^{2})");
         
         
	ca->Print("diMuon_mass_2011_pp_2.png");
