...
void forest2dimuontotalfitpp2011(
		TString fname = "2011ppcase.root",
	...
	for (int i=1;i<nMu;i++){
        ...
        // Crystal ball 
        RooRealVar mean_jpsi("mean_jpsi","mean_jpsi", 3.096916, 2.6, 4); // valor do pico no x
		RooRealVar sigma_jpsi("sigma_jpsi","sigma_jpsi", 0.035, 0, 0.1); // largura da gaussiana 
		...
		RooRealVar sigma_psi2s("sigma_psi2s","sigma_psi2s", 0.08, 0, 0.1);//0.07
		RooGaussian *psi2s = new RooGaussian("psi2s", "Gaussiana", x, mean_psi2s, sigma_psi2s);
	
        //Exponential background
 	 	RooRealVar a("a", "a", -0.05, -5, 5);
		...
		RooRealVar NJpsi("NJpsi", "Numero de eventos de J/Psi",0, 1800); //1800
		RooRealVar NPsi2S("NPsi2S", "Numero de eventos de Psi(2S)", 0, 100);//100
		RooRealVar Nbkg("Nbkg", "Numero de eventos de background", 0, 700);//700
 	 
 	 	RooRealVar fsig("fsig", "signal", 0.82, 0., 1.); // aumentar o pico da CB
        ...
        RooPlot* mframe = x.frame(Title("Fit of invariant mass in 2011 pp collision"));
 	 	dh.plotOn(mframe); 
		...
		/* 
        // Adicionando a legenda ao grafico    
        ...
        latex->DrawLatex(0.15, 0.85, "CMS Open Data");
    
        latex->SetTextSize(0.030); // para deixar o resto com outro tamanho de fonte
        latex->DrawLatex(0.15, 0.78, "pp 2011 #sqrt{s_{NN}} = 2.76 TeV");
        latex->DrawLatex(0.15, 0.74, "P^{#mu}_{T} > 0.0 GeV");
        latex->DrawLatex(0.15, 0.70, "#lbar#eta#lbar < 2.4");
        latex->DrawLatex(0.15, 0.66, "Trigger: HLT_HIL1DoubleMu0");
		*/

	ca->Print("diMuon_mass_2011_pp_5.png");
